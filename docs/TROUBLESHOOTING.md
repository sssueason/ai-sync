...
```
tick <机器> <时间> pull=N commit=N push=N skip=N elapsed=Ns rc=N | <渲染器末行 ×3> [| conv: changed=N …]
```

- `rc=0` = 正常；`rc≥1` = 有失败，**同一行的尾部就是失败的证据**。
- `[FAIL] render <名字> 退出码 N（末行输出：…）` = 渲染器失败，括号里是它自己的诊断行。
- `lock=busy（另一同步进程持锁，本轮让路）` = 让路，**不是故障**。
