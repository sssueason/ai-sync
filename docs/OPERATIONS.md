...
## 10. 怎么更新引擎（每台机器各自 pull）

引擎装在**每台机器自己的 clone** 里，**不会自动更新** —— 自动更新会在任务运行途中替换正在执行的代码，
风险不值当。所以策略是"看得见 + 一键/一条命令"：

- `sync-status` 会报「引擎代码与远端一致」：落后 N 个提交即 **WARN**，并把命令写在「实际」列；
  控制台「总览」顶部出现更新横幅；托盘悬浮第二行也会提示「引擎可更新（落后 N）」。
- 每台机器的心跳带 `engineRev` / `engineBehind` ⇒ 控制台「其他机器」能直接看出**谁还在跑旧代码**。

**一键更新（推荐）**：托盘/菜单栏右键 →「检查并更新引擎」（落后时标签写着「更新引擎（落后 N 个提交）」）。
它一次做完四件事并把结论弹出来：

1. `git fetch` → 判定落后几个提交；
2. `git pull --ff-only`（**绝不** `--force`：本地有分叉或未提交改动就失败并如实报告）；
3. 按"改了哪一片"决定后续 —— 见下面那张表（安装器有变 → 自动重注册；托盘自身有变 → 自动重启托盘）；
4. 弹窗写清 `旧 rev → 新 rev`、拉了几个提交、做了哪些后续动作。

命令行等价形式（无 UI、可测）：`pwsh -File apps\sync-tray\windows\sync-tray.ps1 -UpdateEngine [-DryRun]`。
所有网络调用都带超时（fetch 45s / pull 90s）—— 菜单一点就把 UI 冻住是不可接受的。

不想用一键，也可以手动：

```bash
git -C <引擎根> pull          # <引擎根> 默认 ~/.ai-sync/engine
```
...
| 这次更新涉及 | 还要做什么 |
|---|---|
| 工具 / 适配器 / 文档（`tools/`、`adapters/`、`docs/`） | 不用做别的，**下一个 tick 就用新代码** |
| 调度或安装器（`install/`、任务动作、命令文件） | 再注册一次：`node <引擎根>/install/install.mjs --instance <实例根> --register` |
| 托盘 / 菜单栏（`apps/sync-tray/`） | Windows：重启一次托盘脚本；macOS：`bash <引擎根>/apps/sync-tray/macos/build.sh --install` |

> `--no-fetch` 的调用方（例如菜单栏每 2 分钟的定时刷新）拿不到网络比对结果，此时这条检查如实报
> **INFO「未比对」**而不是假装最新；打开托盘菜单时会带 `fetch` 复核一次，控制台与状态推送也都会比对。
