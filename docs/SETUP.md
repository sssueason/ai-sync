...
1. **会装什么**：一个后台定时任务（每几分钟自动同步一次）+ 一个托盘图标（Windows 右下角 / macOS 顶部菜单栏）+ 一个本地网页控制台。
   没有服务端、没有账号、不对外开放端口。
2. **装完怎么用**：平时只看托盘的角标（绿=正常 / 黄=提醒 / 红=要处理），双击打开控制台看细节。
   **所有设置都在控制台里点**，本文档只在"装"的时候需要。
3. **出错了看哪里**：控制台「总览」的 FAIL 行 →「运行记录」最后一行的备注 → 本目录 [TROUBLESHOOTING.md](TROUBLESHOOTING.md)。
   本文档每条命令下面都写了"期望看到什么"，对不上再往下查。

术语看不懂时翻仓库 [README 的「名词小抄」](../README.md#名词小抄)（tick / 适配器 / 渲染 / 收敛 / 对齐 / µ1 / µ2，各一句话）。
